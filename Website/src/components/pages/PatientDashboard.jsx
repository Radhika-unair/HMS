...
const PatientDashboard = () => {
  ...
  useEffect(() => {
    ...
    const userPrescriptions = allPrescriptions.filter(pres => pres.patientId === user.id);
    setPrescriptions(userPrescriptions);

    // Load lab tests
    const allLabTests = JSON.parse(localStorage.getItem("labTests") || "[]");
    const userLabTests = allLabTests.filter(test => test.patientId === user.id);
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {appointments.map((appointment) => (
                    <tr key={appointment.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">{appointment.isVideo ? "Video Call" : "In-Person"}</div>
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {prescriptions.map((prescription) => (
                <div key={prescription.id} className="border rounded-lg p-4 hover:shadow-md transition-shadow">
                  ...
                  <p className="mt-4 text-sm text-gray-600">{prescription.notes}</p>
                </div>
              ))}
            </div>
          )}
